let planetasRegistrados = [];
function createPlanet(nome, coordenadas, situacao, satelites) {
    let novoPlaneta = {
        nome,
        coordenadas,
        situacao,
...
function acharPlaneta(nomePlaneta) {
    return planetasRegistrados.find((planeta) => planeta.nome === nomePlaneta);
}
function alterarSituacao(nomePlaneta, novaSituacao) {
...
function adcionarSatelite(nome, nomePlaneta) {
    let planeta = acharPlaneta(nomePlaneta);
    if (planeta) {
...
function removerSatelite(nome, nomePlaneta) {
    let planeta = acharPlaneta(nomePlaneta);
    ...
    do {
        ...
        switch (opcao) {
            case 1:
                ...
                let coordenadasPegar = [0, 0, 0, 0]; // Inicialize as coordenadas
                ...
                let situacao;
                do {
                    situacao = prompt('Qual a situação do planeta? (habitado, habitavel, inabitavel, inexplorado)');
