...
interface Espaconave {
  ...
  let espaconaves: Espaconave[] = [];

function novaNaveFunc(nome: string, piloto: string, limiteTrip: number) {
  let novaNave = {
    nome,
    piloto,
    limiteTrip,
...
function acharNave(nomeNave: string) {
  return espaconaves.find((nave) => nave.nome === nomeNave);
}

function adicionarTripulante(nome: string, espaconave: string) {
...
function enviarParaMissao(nomeNave: string) {
